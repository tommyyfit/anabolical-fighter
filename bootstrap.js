(()=>{
  ...
  (async()=>{
    try{
      ...
      const texts=await Promise.all(parts.map(n=>fetch(`v7art/${n}.b64?v=6`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`${n}: HTTP ${r.status}`);return r.text()})));
      ...
      try{
        ...
        console.info(`V7 atlas decoded: ${atlas.naturalWidth}x${atlas.naturalHeight}, ${total} bytes, ${Object.keys(A).length} assets`);
      }finally{URL.revokeObjectURL(atlasUrl)}
      bind();
      setStatus('Starting Iron Circuit…');
      for(const src of ['game-data-1.js?v=6','game-data-2.js?v=6','game-data-3.js?v=6','game-loader.js?v=6'])await load(src);
