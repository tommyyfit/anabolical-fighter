(()=>{
  ...
  (async()=>{
    try{
      ...
      const texts=await Promise.all(parts.map(n=>fetch(`v7art/${n}.b64?v=4`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`${n}: HTTP ${r.status}`);return r.text()})));
      ...
      for(const src of ['game-data-1.js?v=4','game-data-2.js?v=4','game-data-3.js?v=4','game-loader.js?v=4'])await load(src);
